...
The system SHALL provide comprehensive documentation for all public APIs and core features.

#### Scenario: Quickstart Guide
- **WHEN** a developer reads the Quickstart guide
- **THEN** they can successfully connect to the database, define a model, and perform basic CRUD operations without looking at source code

#### Scenario: API Reference
- **WHEN** a developer looks up a public exported function (e.g., `pg_transaction` or `selectinload`)
- **THEN** there is a dedicated documentation entry explaining its parameters, return type, and usage example
