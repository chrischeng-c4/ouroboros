//! WebSocket support for ouroboros-api
//!
//! This module provides WebSocket functionality for real-time bidirectional
//! communication between client and server. It follows the ouroboros
...
//!
//! ```rust,no_run
//! use ouroboros_api::websocket::{WebSocket, is_websocket_upgrade};
//! use ouroboros_api::request::SerializableRequest;
//! use hyper::{Request, Body};
//!
//! # async fn example() -> Result<(), Box<dyn std::error::Error>> {
//! // Check if request is a WebSocket upgrade
//! let req = SerializableRequest::new(
//!     ouroboros_api::request::HttpMethod::Get,
//!     "/ws"
//! )
...
///
/// ```rust,no_run
/// use ouroboros_api::websocket::{WebSocket, WebSocketMessage};
///
...
impl WebSocket {
    ...
    ///
    /// ```rust,no_run
    /// use ouroboros_api::websocket::WebSocket;
    ...
    ///
    /// ```rust,no_run
    /// # use ouroboros_api::websocket::{WebSocket, CloseFrame};
    /// # async fn example(mut ws: WebSocket) -> Result<(), Box<dyn std::error::Error>> {
    /// // Normal closure
    /// ws.close(None).await?;
    ///
...
///
/// ```rust
/// use ouroboros_api::request::{SerializableRequest, HttpMethod};
/// use ouroboros_api::websocket::is_websocket_upgrade;
///
...
///
/// ```rust
/// use ouroboros_api::websocket::generate_accept_key;
///
