...
//!
//! This example demonstrates how to create a basic HTTP server using data-bridge-api.
//!
//! Run with:
//! ```bash
...
fn register_routes(router: &mut Router) -> Result<(), Box<dyn std::error::Error>> {
    ...
    router.route(
        ...
        Arc::new(|_req, _validated| {
            Box::pin(async move {
                use data_bridge_api::Response;
                Ok(Response::json(SerializableValue::Object(vec![
                    (
                        "message".to_string(),
                        SerializableValue::String("Welcome to data-bridge-api!".to_string()),
                    ),
                    (
                        "version".to_string(),
