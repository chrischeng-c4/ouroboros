...
//!
//! Run with: cargo run -p data-bridge-api --example openapi_example

use data_bridge_api::openapi::*;
use data_bridge_api::request::HttpMethod;
use data_bridge_api::validation::{TypeDescriptor, StringConstraints, NumericConstraints, FieldDescriptor};

fn main() {
    // Create a new OpenAPI spec
    let mut spec = OpenApiSpec::new("My API", "1.0.0")
        .description("A sample API built with data-bridge-api")
