...
//!
//! Provides Python bindings for the ouroboros-http crate,
//! following the same patterns as mongodb.rs.

use ouroboros_http::{HttpClient, HttpClientConfig, HttpResponse as RustResponse};
use pyo3::prelude::*;
...
impl PyHttpClient {
    ...
    fn request<'py>(
        ...
        future_into_py(py, async move {
            use ouroboros_http::request::{ExtractedAuth, ExtractedRequest, HttpMethod};
            use std::str::FromStr;
            ...
            let response = client.execute(request).await.map_err(|e| {
                ...
                match e.category() {
                    ouroboros_http::error::HttpErrorCategory::Connection => {
                        PyErr::new::<pyo3::exceptions::PyConnectionError, _>(sanitized)
                    }
                    ouroboros_http::error::HttpErrorCategory::Timeout => {
...
fn extract_body(
    ...
    json: Option<&Bound<'_, PyAny>>,
    form: Option<&Bound<'_, PyDict>>,
) -> PyResult<ouroboros_http::request::ExtractedBody> {
    use ouroboros_http::request::ExtractedBody;
