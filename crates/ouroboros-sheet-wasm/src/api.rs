...
use ouroboros_sheet_formula::{extract_references, DependencyGraph};
use ouroboros_sheet_history::{
...
impl From<&CellFormat> for CellFormatData {
    fn from(format: &CellFormat) -> Self {
        use ouroboros_sheet_core::{HorizontalAlign, VerticalAlign};
...
impl SpreadsheetEngine {
    ...
    fn recalculate_cell(&mut self, row: u32, col: u32) {
        ...
        if let Some(expression) = expression {
            ...
            let result = ouroboros_sheet_formula::evaluate_formula_cross_sheet(
                &expression,
    ...
    pub fn search(&self, options_json: &str) -> Result<String, JsValue> {
        let options: ouroboros_sheet_core::SearchOptions = serde_json::from_str(options_json)
            .map_err(JsRuSheetError::from_error)?;

        let results = ouroboros_sheet_core::SearchEngine::search(&self.workbook, &options)
            .map_err(JsRuSheetError::from_error)?;
    ...
    pub fn replace(&mut self, options_json: &str) -> Result<String, JsValue> {
        let options: ouroboros_sheet_core::ReplaceOptions = serde_json::from_str(options_json)
            .map_err(JsRuSheetError::from_error)?;

        let results = ouroboros_sheet_core::SearchEngine::replace(&mut self.workbook, &options)
            .map_err(JsRuSheetError::from_error)?;
    ...
    pub fn search_current_sheet(&self, query: &str, match_case: bool) -> Result<String, JsValue> {
        let options = ouroboros_sheet_core::SearchOptions {
...
fn cell_format_from_data(data: &CellFormatData) -> CellFormat {
    use ouroboros_sheet_core::{Color, HorizontalAlign, VerticalAlign};
...
#[cfg(test)]
mod bug_fixes {
    use ouroboros_sheet_core::{CellCoord, Sheet};
    ...
    fn test_bug_1_3_number_preservation() {
        ...
        let content = ouroboros_sheet_core::sheet::parse_cell_input("10");
        let cell = ouroboros_sheet_core::Cell::new(content);
        ...
        let formula_content = ouroboros_sheet_core::sheet::parse_cell_input("=B2");
        let formula_cell = ouroboros_sheet_core::Cell::new(formula_content);
    ...
    fn test_original_input_preserved_for_all_types() {
        ...
        // Test number
        let content = ouroboros_sheet_core::sheet::parse_cell_input("42");
        sheet.set_cell(CellCoord::new(0, 0), ouroboros_sheet_core::Cell::new(content));
        let cell = sheet.get_cell(CellCoord::new(0, 0)).unwrap();
        assert_eq!(cell.content.original_input(), "42");

        // Test percentage
        let content = ouroboros_sheet_core::sheet::parse_cell_input("50%");
        sheet.set_cell(CellCoord::new(0, 1), ouroboros_sheet_core::Cell::new(content));
        ...
        // Test boolean
        let content = ouroboros_sheet_core::sheet::parse_cell_input("TRUE");
        sheet.set_cell(CellCoord::new(0, 2), ouroboros_sheet_core::Cell::new(content));
        let cell = sheet.get_cell(CellCoord::new(0, 2)).unwrap();
        assert_eq!(cell.content.original_input(), "TRUE");

        // Test text
        let content = ouroboros_sheet_core::sheet::parse_cell_input("Hello");
        sheet.set_cell(CellCoord::new(0, 3), ouroboros_sheet_core::Cell::new(content));
        let cell = sheet.get_cell(CellCoord::new(0, 3)).unwrap();
        assert_eq!(cell.content.original_input(), "Hello");

        // Test formula
        let content = ouroboros_sheet_core::sheet::parse_cell_input("=A1+B1");
        sheet.set_cell(CellCoord::new(0, 4), ouroboros_sheet_core::Cell::new(content));
    ...
    fn test_data_validation_integration() {
        use ouroboros_sheet_core::{DataValidationRule, ValidationCriteria, ValidationAlert, AlertStyle, ListSource, CellRange, CellCoord};
    ...
    fn test_data_validation_number_range() {
        use ouroboros_sheet_core::{DataValidationRule, ValidationCriteria, ValidationAlert, AlertStyle, ValidationOperator, CellRange, CellCoord};
    ...
    fn test_conditional_formatting_integration() {
        use ouroboros_sheet_core::{CellRange, Color, ComparisonOperator, ConditionalFormat, ConditionalFormattingRule, ConditionalRule};
