...
fn run_throughput_test<F>(
    ...
    engine: &Arc<KvEngine>,
    test_fn: F,
) -> data_bridge_test::benchmark::BenchmarkResult
where
    F: Fn(usize, usize, &Arc<KvEngine>) -> (f64, f64),
{
    ...
    let stats = data_bridge_test::benchmark::BenchmarkStats::from_times(
        times_ms,
        1,
        config.rounds,
        config.warmup,
    ...
    data_bridge_test::benchmark::BenchmarkResult::success(name, stats)
}
