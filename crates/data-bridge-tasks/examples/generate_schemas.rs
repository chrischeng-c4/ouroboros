...
//! Run with:
//! ```bash
//! cargo run -p data-bridge-tasks --features "schema" --example generate_schemas
//! ```

#[cfg(feature = "schema")]
use data_bridge_tasks::schema::{generate_all_schemas, generate_asyncapi};

#[cfg(feature = "schema")]
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== Generating JSON Schemas for data-bridge-tasks ===\n");
...
fn main() {
    ...
    eprintln!("Run with: cargo run -p data-bridge-tasks --features schema --example generate_schemas");
    std::process::exit(1);
}
